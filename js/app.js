...
class App {
    ...
    initializeUIState() {
        ...
        const matchesPerPlayer = uiManager.updateMatchesPerPlayerOptions(playerCount);
    ...
    handleMatchesPerPlayerChange() {
        const playerCount = parseInt(uiManager.elements.playerCount.value);
        const matchesPerPlayer = parseInt(uiManager.elements.matchesPerPlayer.value);
        tournamentManager.matchesPerPlayer = matchesPerPlayer;
        uiManager.updateTournamentInfo(playerCount, matchesPerPlayer);
    }
    ...
    async handleJoinSubmit(event) {
        ...
        try {
            ...
            uiManager.showError('joinError', error.message || 'Failed to join tournament');
        } finally {
    ...
    async attemptRejoin() {
        ...
        const timestamp = parseInt(localStorage.getItem(keys.SESSION_TIMESTAMP) || '0');
